...
> = {
  ...
  breakpoints: {
    ...
    classes: 'grid gap-4 grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 sm:gap-6',
    naiveCards: false,
  },
  autofit: {
    ...
    classes: 'grid gap-4 sm:gap-6 [grid-template-columns:repeat(auto-fill,minmax(16rem,1fr))]',
    naiveCards: false,
  },
}
...
function Notes() {
  ...
  return (
    <section className="mt-10 rounded-xl border border-slate-200 bg-white p-4 sm:p-6">
      <h2 className="text-lg font-semibold text-ink">What to change on the real site</h2>
      <dl className="mt-4 grid gap-x-8 gap-y-5 md:grid-cols-2">
        {items.map(([term, def]) => (
          <div key={term} className="min-w-0">
            <dt className="font-mono text-sm font-semibold text-nord-dark">{term}</dt>
            <dd className="mt-1 text-sm leading-relaxed text-slate-600">{def}</dd>
          </div>
        ))}
      </dl>
    </section>
      ...
      <main className="mx-auto max-w-[1400px] px-4 py-6 sm:px-6 sm:py-8">
        <div className="mb-6">
          ...
          <p className="mt-1 max-w-2xl text-sm text-slate-600">
            Responsive-layout prototype for the card grid and tag rows. Drag the window narrow and
            wide, and watch the breakpoint readout in the bottom-right corner.
          </p>
        </div>

        <Toolbar layout={layout} setLayout={setLayout} clamp={clamp} setClamp={setClamp} />
